...
function adding_window(){
  ...
  if (!wind){// checking if the window exist already in the markup
    ...
    wind += "<label>Type</label><div class='radio radio-inline'><label><input type='radio' name='type'>Consomable</label></div>";
    ...
    wind += "<button class='btn btn-default red' type='button' onclick='cancel_add();'>Annuler</button>";
    wind += "<button class='btn btn-default green' type='button' onclick='add_product();'>Ajouter</button>";
    wind += "</div></div>";
